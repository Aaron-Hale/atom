...
**Tests**: Include test tasks whenever behavior changes. For retrieval/reranking, evidence,
schema, or evaluation workflow changes, tests are REQUIRED and MUST fail before implementation.
...
- [ ] T004 [P] Define deterministic execution inputs (seed policy, dataset/split versions, config IDs)
...
- [ ] T011 Establish baseline evaluation harness and output format in `docs/report_*.md`
...
- [ ] T014 [P] [US1] Regression test for evidence span/provenance fidelity
...
- [ ] TXXX Run baseline vs improved evaluation and record deltas in `docs/report_*.md`
...
- Required tests for changed behavior MUST be written and FAIL before implementation
...
- Do not claim improvement without baseline-vs-improved metrics on same protocol
